...
sally_table <- table(sally_chi$section, sally_chi$unittype)
```
...
```
###### Answer: A chi-square test revealed a significant relationship between forest type and channel unit for Coastal giant salamander abundance (x-squared = 200.71, df = 5, p < 0.001). This suggests that salamander abundance varies by habitat type in different forest conditions.
...
t.test(log(weight_g) ~ section, data = sally_clean, var.equal = TRUE)
```
###### Answer: A two sample t-test showed a significant difference in Coastal Giant Salamander biomass between clear cut and old growth forest sections (t = 10.09, df = 6336, p < 0.001). The mean log-transformed weight was higher in clear cut forests (1.83) compared to old growth forests (1.58). This suggests salamanders were significantly larger in clear cut areas.
...
```{r}
# Clean trout data
trout_corr <- and_vertebrates |> 
  filter(species == "Cutthroat trout") |> 
  drop_na(length_1_mm, weight_g)
```

```{r}
# correlation test with log transform
cor.test(log(trout_corr$length_1_mm), log(trout_corr$weight_g))
```
###### Answer: A correlation test showed a strong, positive relationship between body length and weight in Cutthroat trout (r = 0.99, p < 0.001). This result supports the hypothesis that longer individuals also tend to weigh more.
...
```{r}
# Subset data to 3 sites
pie_sites <- pie_crab |> 
  filter(site %in% c("GTM", "DB", "PIE"))
```

```{r}
# Create a factor that orders sites by mean latitude
site_order <- pie_sites |> 
  group_by(site) |> 
...
```{r}
# Reorder factor levels in site column
pie_sites$site <- factor(pie_sites$site, levels = site_order)
```

```{r}
# Create boxplot
ggboxplot(pie_sites, x = "site", y = "size", fill = "site") +
  geom_jitter(width = 0.1, alpha = 0.4) +
  theme_minimal() +
  labs(title = "Carapace Width by Site (Ordered by Latitude)",
       x = "Site", y = "Carapace Width (mm)")
```
###### Answer: There was a significant difference in crab size among the three sites (GTM, DB, and PIE) as shown by an ANOVA test (p < 0.001). The Tukey post-hoc test showed that all sites were significantly different from each other, with crabs being smallest at GTM, medium at DB, and largest at PIE.
...
```{r}
lm_water_sd <- lm(size ~ water_temp_sd, data = pie_crab)
summary(lm_water_sd)
```

```{r}
# Plot
ggscatter(pie_crab, x = "water_temp_sd", y = "size",
          add = "reg.line", conf.int = TRUE,
          color = "#0072B2", alpha = 0.6) +
  labs(title = "Effect of Water Temp SD on Crab Size",
       x = "Water Temp SD", y = "Carapace Width (mm)")
```
###### Answer: I ran a linear regression to test the effect of water temperature variability on crab size. The p-value was 0.527 (p > 0.05), so we found no significant relationship between water temperature SD and carapace width. The plot also shows no clear trend in crab size across different temperature values.
...
```{r}  
mlm <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)
summary(mlm)  
```
###### Answer: A multiple linear regression showed that latitude had a significant effect on crab size (p < 0.001), while air temperature SD (p = 0.17) and water temperature SD (p = 0.33) did not. The overall model was significant (p < 0.001) and explained about 35% of the variation in crab size (r-squared = 0.35).
